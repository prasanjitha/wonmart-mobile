...
class _CreateOrderScreenState extends State<CreateOrderScreen> {
  ...
  double get _total => _orderRows.fold(0, (sum, item) => sum + item.totalAgentPrice);
  double get _totalProfit => _orderRows.fold(0, (sum, item) => sum + item.totalProfit);
  ...
  void _addItem() {
    ...
    final agentPrice = retailPrice * (1 - margin / 100);
  ...
  void _updateRow(int index, {int? quantity, double? price, double? margin}) {
    setState(() {
      final item = _orderRows[index];
      final updatedQuantity = quantity ?? item.quantity;
      final updatedPrice = price ?? item.price; // Retail Price
      final updatedMargin = margin ?? item.marginPercentage;

      final updatedAgentPrice = updatedPrice * (1 - updatedMargin / 100);
      final updatedTotalRetailPrice = updatedQuantity * updatedPrice;
      final updatedTotalAgentPrice = updatedQuantity * updatedAgentPrice;
      final updatedTotalProfit = updatedTotalRetailPrice - updatedTotalAgentPrice;
  ...
  Future<void> _showEditDialog(int index) async {
    ...
    final marginCtrl = TextEditingController(text: item.marginPercentage.toString());
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              final m = double.tryParse(marginCtrl.text) ?? item.marginPercentage;
              _updateRow(index, quantity: q, price: p, margin: m);
